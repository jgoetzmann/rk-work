...
void rk_step(int16_t *y, int16_t h_q)
{
    ...
    for (m = 0; m < N_STATES; m++) {
        tmp = (int32_t)y[m];                     /* load */
        /* 1/8 ~ 1 / 2^3 (exact, csd weight 1) */
        tmp += ((int32_t)hk[0][m] * (int32_t)1) >> 3;
        /* 3/8 ~ 3 / 2^3 (exact, csd weight 2) */
        tmp += ((int32_t)hk[1][m] * (int32_t)3) >> 3;
        /* 3/8 ~ 3 / 2^3 (exact, csd weight 2) */
        tmp += ((int32_t)hk[2][m] * (int32_t)3) >> 3;
        /* 1/8 ~ 1 / 2^3 (exact, csd weight 1) */
        tmp += ((int32_t)hk[3][m] * (int32_t)1) >> 3;
