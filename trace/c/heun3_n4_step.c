...
void rk_step(int16_t *y, int16_t h_q)
{
    ...
    for (m = 0; m < N_STATES; m++) {
        ...
        tmp += ((int32_t)hk[0][m] * (int32_t)21845) >> 16;
    ...
    for (m = 0; m < N_STATES; m++) {
        tmp = (int32_t)y[m];                     /* load */
        /* 2/3 ~ 21845 / 2^15 (approx, csd weight 8) */
        tmp += ((int32_t)hk[1][m] * (int32_t)21845) >> 15;
        acc[m] = (int16_t)tmp;                   /* store */
    }
    rk_rhs(acc, k);
    for (m = 0; m < N_STATES; m++) {
    ...
    for (m = 0; m < N_STATES; m++) {
        tmp = (int32_t)y[m];                     /* load */
        /* 1/4 ~ 1 / 2^2 (exact, csd weight 1) */
        tmp += ((int32_t)hk[0][m] * (int32_t)1) >> 2;
        /* 3/4 ~ 3 / 2^2 (exact, csd weight 2) */
        tmp += ((int32_t)hk[2][m] * (int32_t)3) >> 2;
